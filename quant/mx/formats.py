...
def _get_format_params(fmt):
    ...
    elif fmt == ElemFormat.lns8_e4m3:
        ebits, mbits = 4, 3
        emax = 2**(ebits - 1) - 1  # 7
        max_norm = 2 ** (emax + (1 - 1 / (2 ** mbits)))  # 2 ** 7.875
        min_norm = 2 ** (-emax - (1 - 1 / (2 ** mbits)))  # 2 ** -7.875
        _FORMAT_CACHE[fmt] = (ebits, mbits, emax, max_norm, min_norm)
        return ebits, mbits, emax, max_norm, min_norm

    elif fmt == ElemFormat.lns8_e3m4:
        ebits, mbits = 3, 4
        emax = 2**(ebits - 1) - 1  # 3
        max_norm = 2 ** (emax + (1 - 1 / (2 ** mbits)))  # 2 ** 3.9375
        min_norm = 2 ** (-emax - (1 - 1 / (2 ** mbits)))  # 2 ** -3.9375
        _FORMAT_CACHE[fmt] = (ebits, mbits, emax, max_norm, min_norm)
        return ebits, mbits, emax, max_norm, min_norm
    
    elif fmt == ElemFormat.lns4_e2m1:
        ebits, mbits = 2, 1
        emax = 2**(ebits - 1) - 1
        max_norm = 2 ** 1.5
        min_norm = 2 ** -1.5
    ...
    elif fmt == ElemFormat.lns4_e3m0:
        ebits, mbits = 3, 0
        emax = 2**(ebits - 1) - 1  # 3
        max_norm = 2 ** (emax + 1 - 0)  # 2^4 = 16
        min_norm = 2 ** (-emax - 1 + 0)  # 2^-4 = 0.0625
        _FORMAT_CACHE[fmt] = (ebits, mbits, emax, max_norm, min_norm)
        return ebits, mbits, emax, max_norm, min_norm

    elif fmt == ElemFormat.lns4_e1m2:
        ebits, mbits = 1, 2
        emax = 2**(ebits - 1) - 1  # 0
        max_norm = 2 ** (emax + 1 - 0.25)  # 2^0.75 ≈ 1.681
        min_norm = 2 ** (-emax - 1 + 0.25)  # 2^-0.75 ≈ 0.595
        _FORMAT_CACHE[fmt] = (ebits, mbits, emax, max_norm, min_norm)
        return ebits, mbits, emax, max_norm, min_norm

    elif fmt == ElemFormat.lns4_e0m3:
        ebits, mbits = 0, 3
        emax = 0
        max_norm = 2 ** (0 + 1 - 0.125)  # 2^0.875 ≈ 1.828
        min_norm = 2 ** (0.125)  # 0.125 ≈ 0.544
        _FORMAT_CACHE[fmt] = (ebits, mbits, emax, max_norm, min_norm)
        return ebits, mbits, emax, max_norm, min_norm

    elif fmt == ElemFormat.lns4_e1m2_v0:
        ebits, mbits = 1, 2
        emax = 2
        max_norm = 2 ** (emax + 1 - 0.25)  # 2^0.75 ≈ 1.681
        min_norm = 2 ** (-emax - 1 + 0.25)  # 2^-0.75 ≈ 0.595
    ...
    elif fmt == ElemFormat.lns4_e2m1_v0:
        ebits, mbits = 2, 1
        emax = 2**(ebits - 1) - 1
        max_norm = 2 ** 1.25
        min_norm = 2 ** -1.25
    ...
    elif fmt == ElemFormat.lns4_e2m1_v1:
        ebits, mbits = 2, 1
        emax = 4
        max_norm = 2 ** 4.5
        min_norm = 2 ** -4.5
