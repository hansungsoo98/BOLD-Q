...
from .mx_ops import quantize_mx_op, quantize_mx_op_matmul_A
...
class LinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        # Skip extra weight quantization for selected LM head shapes.
        llama2_7b_lm_head_shape = torch.Size([32000, 4096])
        llama3_target_lm_head_shape = torch.Size([128256, 2048]) 
        opt6dot7_target_lm_head_shape = torch.Size([50272, 4096]) 

        is_target_lm_head = (weight.shape == llama2_7b_lm_head_shape or 
                            weight.shape == llama3_target_lm_head_shape or
                            weight.shape == opt6dot7_target_lm_head_shape)
        ...
        # print("qis_input")
        qis_input = quantize_mx_op_matmul_A(
            bf_in,
            mx_specs,
        ...
        if mx_specs["prequantized"] or is_target_lm_head :
            qis_weight = weight  
        ...
        if qis_input.dtype != qis_weight.dtype:
        ...
        # noise injection!!
        if not is_target_lm_head :
            output = inject_blockwise_mul_noise_torch(
                out=output,
